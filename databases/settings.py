...
def init_settings() -> None:
    with _connect() as connection:
        connection.execute(
            "CREATE TABLE IF NOT EXISTS settings (guild_id INTEGER NOT NULL, key TEXT NOT NULL, value TEXT NOT NULL, PRIMARY KEY (guild_id, key))"
        )
        connection.execute(
            "CREATE TABLE IF NOT EXISTS settings_audit (id INTEGER PRIMARY KEY AUTOINCREMENT, guild_id INTEGER NOT NULL, user_id INTEGER NOT NULL, key TEXT NOT NULL, old_value TEXT, new_value TEXT NOT NULL, created_at TEXT NOT NULL)"
        )
        connection.commit()
...
def set_setting(guild_id: int, user_id: int, key: str, value) -> tuple[bool, str, str]:
    ...
    with _connect() as connection:
        ...
        connection.execute(
            ...
            (guild_id, key, new_value),
        )
        connection.execute(
            "INSERT INTO settings_audit (guild_id, user_id, key, old_value, new_value, created_at) VALUES (?, ?, ?, ?, ?, ?)",
            (guild_id, user_id, key, old_value, new_value, datetime.now(timezone.utc).isoformat()),
        )
        connection.commit()
