...
class CreateVoice(commands.Cog):
    ...
    async def on_voice_state_update(
        ...
        if join_channel and create_voice_channel_id and join_channel.id == create_voice_channel_id:
            async with self._guild_locks[member.guild.id]:
                ...
                try:
                    ...
                    logger.error(
                        "Bot doesn't have permission to create voice channels in guild %s",
                        member.guild.id,
                    )
                    return
                except disnake.HTTPException as e:
                    logger.exception(
                        "Failed to create voice channel for member %s: %s", member.id, e
                    )
                    return
                ...
                try:
                    ...
                    logger.warning(
                        "Bot doesn't have permission to set permissions for channel %s",
                        new_channel.id,
                    )
                except disnake.HTTPException as e:
                    logger.exception(
                        "Failed to set permissions for channel %s: %s", new_channel.id, e
                    )
        ...
        if (
            before.channel
            and before.channel != join_channel
            and before.channel.name.startswith(ARENA_PREFIX)
            ...
            try:
                ...
                logger.exception(
                    "Failed to delete empty channel %s: %s", before.channel.id, e
                )
