...
class AdminPanel(commands.Cog):
    ...
    async def set_log_channel(self, interaction: disnake.ModalInteraction, log_type: str, raw_channel: str) -> None:
        ...
        logger.info("[SETTINGS] %s changed log channel %s -> %s in guild %s", interaction.author.id, log_type, channel_id, interaction.guild.id)
...
class AdminLoggingView(disnake.ui.View):
    ...
    @disnake.ui.select(
        placeholder="Выберите тип логов",
        custom_id="admin:log_select",
        options=[disnake.SelectOption(label=value, value=key) for key, value in LOG_TYPES.items()],
    )
    async def select(self, select: disnake.ui.Select, interaction: disnake.MessageInteraction) -> None:
...
class LogChannelModal(disnake.ui.Modal):
    def __init__(self, cog: AdminPanel, log_type: str) -> None:
        ...
        super().__init__(title=f"Канал: {LOG_TYPES[log_type]}", components=[
            disnake.ui.TextInput(label="ID канала или #канал", custom_id="channel", required=True, max_length=30)
        ])
