...
class GuildLogs(BaseLogger, commands.Cog):
    ...
    async def _audit_entry(
        ...
        """Find a recent audit entry for a specific target without blocking event processing."""
        try:
    ...
    async def on_member_join(self, member: disnake.Member) -> None:
        ...
        embed.add_field(
            ...
            value=f"{disnake.utils.format_dt(member.created_at, 'D')} ({disnake.utils.format_dt(member.created_at, 'R')})",
            inline=True,
        )
    ...
    async def on_member_remove(self, member: disnake.Member) -> None:
        ...
        if member.joined_at:
            embed.add_field(
                ...
                value=f"{disnake.utils.format_dt(member.joined_at, 'D')} ({disnake.utils.format_dt(member.joined_at, 'R')})",
                inline=True,
            )
    ...
    async def on_member_update(self, before: disnake.Member, after: disnake.Member) -> None:
        if before.timed_out_until != after.timed_out_until:
            ...
            if after.timed_out_until:
                embed.add_field(
                    name="Окончание",
                    value=f"{disnake.utils.format_dt(after.timed_out_until, 'f')} ({disnake.utils.format_dt(after.timed_out_until, 'R')})",
                    inline=False,
                )
            ...
            if added_roles or removed_roles:
                ...
                if added_roles:
                    embed.add_field(
                        ...
                        value="\n".join(role.mention for role in added_roles)[:1024],
                        inline=False,
                    )
                if removed_roles:
                    embed.add_field(
                        name=f"Удалены роли ({len(removed_roles)})",
                        value="\n".join(role.mention for role in removed_roles)[:1024],
                        inline=False,
                    )
                await self.log_to_channel(after.guild, embed)
