...
class Moderation(commands.Cog):
    ...
    async def warn(self, inter, member: disnake.Member, reason: str = "Не указана") -> None:
        ...
        await inter.followup.send(f"⚠️ {member.mention} получил предупреждение.", ephemeral=True)
    ...
    async def timeout(self, inter, member: disnake.Member, minutes: int, reason: str = "Не указана") -> None:
        ...
        await inter.followup.send(f"⏱️ {member.mention} получил timeout на {minutes} мин.", ephemeral=True)
    ...
    async def kick(self, inter, member: disnake.Member, reason: str = "Не указана") -> None:
        ...
        await inter.followup.send(f"👢 {member.mention} исключён с сервера.", ephemeral=True)
    ...
    async def ban(self, inter, member: disnake.Member, reason: str = "Не указана") -> None:
        ...
        await inter.followup.send(f"🔨 {member.mention} заблокирован.", ephemeral=True)
    ...
    async def unban(self, inter, user_id: str, reason: str = "Не указана") -> None:
        ...
        try:
            ...
            await inter.followup.send("Пользователь не найден.", ephemeral=True)
            return
        await self._log_action(inter.guild, user, inter.author, "unban", reason)
        await inter.followup.send(f"✅ {user.mention} разблокирован.", ephemeral=True)
    ...
    async def history(self, inter, member: disnake.Member) -> None:
        ...
        if not rows:
            await inter.followup.send("История наказаний пуста.", ephemeral=True)
            return
...
class ModerationTargetModal(disnake.ui.Modal):
    ...
    async def callback(self, inter: disnake.ModalInteraction) -> None:
        ...
        try:
            ...
            await inter.followup.send("Пользователь не найден на сервере.", ephemeral=True)
            return
        ...
        try:
            ...
            await inter.followup.send(f"✅ Действие `{self.action}` выполнено для {member.mention}.", ephemeral=True)
        except RuntimeError as exc:
            await inter.followup.send(f"⚠️ {exc}", ephemeral=True)
        except ValueError:
            await inter.followup.send("Некорректный срок timeout.", ephemeral=True)
        except disnake.Forbidden:
            await inter.followup.send("У бота недостаточно Discord-прав.", ephemeral=True)
